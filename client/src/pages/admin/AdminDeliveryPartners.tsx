...
export default function AdminDeliveryPartners() {
   ...
   const handleSubmit = async (e: React.SubmitEvent) => {
      ...
      try {
         ...
         toast.error(error.response?.data?.message || error?.message || 'Failed to add delivery partner');
      } finally {
         setSaving(false);
      }
   };
   ...
   const toggleActive = async (id: string, isActive: boolean) => {
      try {
         await api.put(`/admin/delivery-partner/${id}`, { isActive: !isActive });
         toast.success('Partner updated successfully!');
         fetchPartners();
      } catch (error: any) {
         toast.error(error.response?.data?.message || error?.message || 'Failed to update delivery partner');
      }
   };
