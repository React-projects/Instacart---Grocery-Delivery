            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                {loading ? (
                    ...
                    <div className="text-center py-16">
                        ...
                        <p className="text-sm  text-app-text-light">Check back soon for Amazing Offer</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4">{products.map(product => product.stock > 0 && <ProductCard key={product._id} product={product} />)}</div>
                )}
            </div>
        </div>
    );
