        ...
        <div className="min-h-screen bg-app-cream mb-20">
            <div className=" max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
                    {tabs.map(tab => (
                        <button onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium rounded-xl whitespace-nowrap transition-colors ${activeTab === tab ? 'bg-app-green text-white' : 'bg-white text-app-text-light hover:bg-app-cream'}`} key={tab}>
                ...
                {loading ? (
                    ...
                    <div className="space-y-4">
                        {orders.map(order => (
                            <Link key={order._id} to={`/orders/${order._id}`} className="block max-w-4xl bg-white rounded-2x; p-5 hover:shadow transition-all">
                                {/* {/ * order id, date & status*/}
                                <div className="flex items-center justify-between mb-3">
                                    {/* left side content */}
                                    <div>
                                        <p className="text-sm text-app-green font-medium">Order # {order._id.slice(-8).toUpperCase()}</p>
