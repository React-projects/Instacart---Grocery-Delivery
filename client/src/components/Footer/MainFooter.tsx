        ...
        <footer className="bg-app-green text-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <div className=" grid grid-col-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {/* brand  */}
                    <div>
                        <Link to="/" className="flex item-center gap-2 mb-4">
                            <BikeIcon className="size-6 text-white" />
                            <span className="text-2xl font-semibold">{footerData.brand.name}</span>
                        </Link>
                ...
                {/* bottom */}
                <div className="border-t border-white/10 sm:flex-row justify-between items-center gap-4 mt-10 pt-6 flex flex-col ">
                    <p className="text-xs text-white/50">{footerData.bottom.copyright}</p>
                    <div className="flex gap-4">
                        {footerData.bottom.links.map((link, index) => {
                            return (
                                <a key={index} href={link.href} className="text-xs text-white/50 hover:text-white/70">
                                    {link.label}
                                </a>
                            );
                        })}
