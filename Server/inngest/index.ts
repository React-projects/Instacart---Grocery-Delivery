...
const checkLowerStock = inngest.createFunction({ id: 'check-low-stock', name: 'Check Low Stock Alert', triggers: [{ event: 'inventory/stock-update' }] }, async ({ event, step }) => {
   ...
   await step.run('low-stock-alert', async () => {
      ...
      if (adminEmails.length === 0) return { skip: true, reason: 'no admin email configured' };
      await sendEmails({
         to: adminEmails.join(','),
...
const sendMonthlyOffers = inngest.createFunction({ id: 'send-monthly-offers', name: 'Send Monthly Payday Offers', triggers: [cron('0 10 1 * *')] }, async ({ step }) => {
   const { deals, users } = await step.run('fetch-deals-and-users', async () => {
      // get top discounted  and featured deals
   ...
   if (users.length === 0 || deals.length === 0) return { skip: true, reason: 'no users or deals found' };
   let sentCount = 0;
   ...
   for (let i = 0; i < users.length; i += batchSize) {
      ...
      await step.run(`send-offers-batch${i}`, async () => {
         for (const user of batch) {
            await sendEmails({
               to: user.email,
               subject: `Free Picks just for you`,
               body: `<div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 520px; margin: auto; border: 1px solid #e5e7eb; border-radius: 16px; overflow: hidden;">
      ...
      return { sent: `sent ${sentCount} emails` };
   }
});
// Auto-Assign Rider after 5 Minutes
const assignRider = inngest.createFunction({ id: 'auto-assign-rider', name: 'auto assign delivery rider', triggers: [{ event: 'order/placed' }] }, async ({ event, step }) => {
   const { orderId } = event.data;
   // watch for 5 minutes before order placed
   await step.sleep('wait-for-5minutes', '5m');
   const result = await step.run('assign-rider', async () => {
      ...
      if (order.deliveryPartnerId) return { skipped: true, reason: 'Rider already assigned' };
      ...
      const busyRidersId = busyOrders.map((o) => o.deliveryPartnerId);

      const availableRider = await prisma.deliveryPartner.findFirst({
         where: {
            isActive: true,
            id: { notIn: busyRidersId as string[] },
         },
      });
      ...
      history.push({
         status: 'Assigned',
         not: `Order assigned to ${availableRider.name}`,
         timestamp: new Date(),
      });
...
export const functions = [checkLowerStock, sendMonthlyOffers, assignRider];
